...
This report summarizes the experiment in \texttt{notebooks/experiment.ipynb}, where the goal is to predict customer churn (\texttt{Churn} = 0/1). Four classification models were trained and compared, then the best model was selected using F1 score.
...
\midrule
\texttt{CustomerID} & Unique identifier for each customer record. Used only for identification and dropped before training. \\
...
\texttt{Total Spend} & Cumulative spending amount by customer. Proxy for customer value and engagement. \\
\texttt{Last Interaction} & Recency/amount of latest interaction activity as provided in dataset. \\
...
\section{Artifacts and Final Output}
...
\section{Recommendation System (\texttt{recommend.ipynb})}
...
\texttt{Usage Frequency} & Increase & +1 session/month & Campaigns/tutorials to increase platform engagement \\
\texttt{Last Interaction} & Decrease & -1 day & Earlier proactive outreach (contact sooner) \\
...
\begin{center}
\texttt{Female, Tenure=27, Usage Frequency=6, Support Calls=4, Payment Delay=11,}\\
\texttt{Subscription Type=Premium, Contract Length=Quarterly, Total Spend=933,}\\
\texttt{Last Interaction=24, AgeGroup=Adult}
\end{center}

Observed output:
\begin{itemize}
    \item Initial prediction: \textbf{Churn} with probability \textbf{98.00\%}
    \item Recommended status: \texttt{SINGLE\_CHANGE}
    \item Minimal intervention: \texttt{Last Interaction} from \texttt{24} to \texttt{15} days
    \item New churn probability after intervention: \textbf{4.00\%}
\end{itemize}
